                        ...
                        case 401:
                            do {
                                ...
                                if message.contains("Your session has expired") {
                                    completion(.failure(NSError.errorTokenExpired(with: message)))
                                } else {
                                    completion(.failure(NSError.error(with: message)))
                                }
                            } catch {
                                print("Failed to decode error response for 401")
...
public extension NSError {
    ...
    static func errorTokenExpired(with localizedDescription: String) -> NSError {
        NSError(domain: "Networking.TokenExpired", code: -1, userInfo: [NSLocalizedDescriptionKey: localizedDescription])
    }

    func isTokenExpiredError() -> Bool {
        domain == "Networking.TokenExpired"
    }
}
